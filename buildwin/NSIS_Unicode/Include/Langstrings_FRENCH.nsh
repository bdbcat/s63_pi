#LangString Installer ${LANG_ENGLISH} "installer"
LangString Installer ${LANG_FRENCH} "installation"

#LangString Uninstaller ${LANG_ENGLISH} "uninstaller"
LangString Uninstaller ${LANG_FRENCH} "désinstallation"

#LangString Installer_Unsupported_Windows_Version ${LANG_ENGLISH} "Your Windows version is not supported (it's too old)!$\r$\nYou need at least Windows XP SP3."
LangString Installer_Unsupported_Windows_Version ${LANG_FRENCH} "Votre version Windows est trop ancienne !$\r$\nVous devez utiliser au moins Windows XP SP3."

#LangString Installer_Running ${LANG_ENGLISH} "The installer is already running."
LangString Installer_Running ${LANG_FRENCH} "L'installation est déjà en cours."

#LangString Uninstaller_Running ${LANG_ENGLISH} "The uninstaller is already running."
LangString Uninstaller_Running ${LANG_FRENCH} "La désinstallation déjà est en cours."

#LangString LangDialog_Title ${LANG_ENGLISH} "OpenCPN 3.3.1117 Installation"
LangString LangDialog_Title ${LANG_FRENCH} "OpenCPN 3.3.1117 Installation"

#LangString LangDialog_Text ${LANG_ENGLISH} "Please select a language:"
LangString LangDialog_Text ${LANG_FRENCH} "Veuillez sélectionner une langue:"

#LangString text_please_wait_1 ${LANG_ENGLISH} "... please wait!"
LangString text_please_wait_1 ${LANG_FRENCH} "... Patientez!"

#LangString Scanning_HDD ${LANG_ENGLISH} "Scanning HDD ..."
LangString Scanning_HDD ${LANG_FRENCH} "Balayage disque ..."

#LangString OCPN_Scanning_HDD ${LANG_ENGLISH} "OpenCPN: Scan HDD ..., please wait!"
LangString OCPN_Scanning_HDD ${LANG_FRENCH} "OpenCPN: balayage disque ...,patientez!"

#LangString Search_old_installations ${LANG_ENGLISH} "Searching for old installations"
LangString Search_old_installations ${LANG_FRENCH} "Recherche anciennes versions"

#LangString Install_Location_Default ${LANG_ENGLISH} "Install Locations by default paths (may not find all)"
LangString Install_Location_Default ${LANG_FRENCH} "Détection dans l'emplacement  par défaut (Peut être incomplète)"

#LangString Install_Location_HDD_Scan ${LANG_ENGLISH} "Install Location by full HDD scan (most reliable)"
LangString Install_Location_HDD_Scan ${LANG_FRENCH} "Détection par balayage du disque (Plus fiable)"

#LangString Label_CleanUp_Text ${LANG_ENGLISH} "Click on a list-item to select/ unselect it or Select ALL (recommended) and then Uninstall selected."
LangString Label_CleanUp_Text ${LANG_FRENCH} "Cliquer sur chaque éléments à désinstaller ou sur 'Sélectionner TOUT' (recommandé) puis lancer la désinstallation "

#LangString BT_Uninstall_Selected_CleanUp ${LANG_ENGLISH} "Uninstall Selected"
LangString BT_Uninstall_Selected_CleanUp ${LANG_FRENCH} "Désinstallation sélectionnée"

#LangString BT_SelectAll_CleanUp ${LANG_ENGLISH} "Select ALL"
LangString BT_SelectAll_CleanUp ${LANG_FRENCH} "Sélectionner TOUT"

#LangString BT_UnselectAll_CleanUp ${LANG_ENGLISH} "Unselect ALL"
LangString BT_UnselectAll_CleanUp ${LANG_FRENCH} "Désélectionner TOUT"

#LangString LB_CleanUp ${LANG_ENGLISH} "Listbox Text"
LangString LB_CleanUp ${LANG_FRENCH} "Texte"

#LangString BT_FullScan_CleanUp ${LANG_ENGLISH} "Scan HDD ..."
LangString BT_FullScan_CleanUp ${LANG_FRENCH} "Balayage disque ..."

#LangString LAB_FullScan_CleanUp ${LANG_ENGLISH} "Scan the HDD for the most reliable detection.$\r$\nATTENTION: may last several minutes!"
LangString LAB_FullScan_CleanUp ${LANG_FRENCH} "Balayage du disque pour une détection plus fiable.$\r$\nATTENTION: celà peut durer plusieurs minutes!"

#LangString Page_CleanUp_Title ${LANG_ENGLISH} "Uninstall older Version"
LangString Page_CleanUp_Title ${LANG_FRENCH} "Désinstalle l'ancienne version"

#LangString Page_CleanUp_Subtitle ${LANG_ENGLISH} "Incompatible version detected"
LangString Page_CleanUp_Subtitle ${LANG_FRENCH} "Version incompatible détectée"

#LangString CP_HEADER_TEXT ${LANG_ENGLISH} "Installation Settings"
LangString CP_HEADER_TEXT ${LANG_FRENCH} "Paramétrage de l'installation"

#LangString CP_HEADER_SUBTEXT ${LANG_ENGLISH} "Choose the settings for the installation"
LangString CP_HEADER_SUBTEXT ${LANG_FRENCH} "Choisir les paramètres pour l'installation"

#LangString CP_TOP ${LANG_ENGLISH} "Check or uncheck the options for the installation.$\r$\nClick 'Next' to continue."
LangString CP_TOP ${LANG_FRENCH} "Choisir les options pour l'installation.$\r$\nCliquer 'Suivant' pour continuer."

#LangString CP_INSTTYPE ${LANG_ENGLISH} "Installation type:"
LangString CP_INSTTYPE ${LANG_FRENCH} "Options d'Installation:"

#LangString CP_COMPLIST ${LANG_ENGLISH} "Adjust the settings individually:"
LangString CP_COMPLIST ${LANG_FRENCH} "Ajuster les paramètres individuellement:"

#LangString Page_SetConfig_Title ${LANG_ENGLISH} "Default configuration settings"
LangString Page_SetConfig_Title ${LANG_FRENCH} "Configuration par défaut"

#LangString Page_SetConfig_Subtitle ${LANG_ENGLISH} "Preconfigure settings for OpenCPN"
LangString Page_SetConfig_Subtitle ${LANG_FRENCH} "Prédéfinir les paramètres pour OpenCPN"

#LangString Select_Chart_Directory ${LANG_ENGLISH} "Please select a chart directory to add to OpenCPN"
LangString Select_Chart_Directory ${LANG_FRENCH} "Choisir un répertoire de cartes à ajouter à OpenCPN"

#LangString BT_Browse_SetConfig_Text ${LANG_ENGLISH} "Browse..."
LangString BT_Browse_SetConfig_Text ${LANG_FRENCH} "Parcourir..."

#LangString BT_DeleteAll_SetConfig_Text ${LANG_ENGLISH} "Delete ALL"
LangString BT_DeleteAll_SetConfig_Text ${LANG_FRENCH} "Supprimer TOUT"

#LangString BT_Delete_SetConfig_Text ${LANG_ENGLISH} "Delete"
LangString BT_Delete_SetConfig_Text ${LANG_FRENCH} "Supprimer"

#LangString GB_ChartDir_SetConfig_Text ${LANG_ENGLISH} "Add Chart Directory"
LangString GB_ChartDir_SetConfig_Text ${LANG_FRENCH} "Ajouter un répertoire de cartes"

#LangString CB_Language_SetConfig_Text ${LANG_ENGLISH} "Set OpenCPN program language to $LANG_NAME"
LangString CB_Language_SetConfig_Text ${LANG_FRENCH} "Conserver cette langue :  $LANG_NAME"

#LangString GB_Language_SetConfig_Text ${LANG_ENGLISH} "Set OpenCPN program language"
LangString GB_Language_SetConfig_Text ${LANG_FRENCH} "Définir la langue pour OpenCPN"

#LangString Label_SetConfig_Text ${LANG_ENGLISH} "Configure preconfig settings for the OpenCPN program.$\r$\nThese settings are saved in the file opencpn.ini."
LangString Label_SetConfig_Text ${LANG_FRENCH} "Prédéfinir les paramètres pour OpenCPN.$\r$\nCes paramètres seront sauvegardés dans le fichier opencpn.ini."

#LangString Page_Confirm_Title ${LANG_ENGLISH} "Ready to Install"
LangString Page_Confirm_Title ${LANG_FRENCH} "Prêt pour l'installation"

#LangString Page_Confirm_Subtitle ${LANG_ENGLISH} "Check the installation options"
LangString Page_Confirm_Subtitle ${LANG_FRENCH} "Verifier les options d'installation"

#LangString Installation_Type ${LANG_ENGLISH} "Setup type"
LangString Installation_Type ${LANG_FRENCH} "Type d'installation"

#LangString Installation_Directory ${LANG_ENGLISH} "Destination location"


#LangString Create_Shortcuts ${LANG_ENGLISH} "Create shortcuts"
LangString Create_Shortcuts ${LANG_FRENCH} "Créer un raccourci"

#LangString Type_Reinstall ${LANG_ENGLISH} "Reinstallation"
LangString Type_Reinstall ${LANG_FRENCH} "Réinstallation"

#LangString Type_Upgrade ${LANG_ENGLISH} "Upgrade"
LangString Type_Upgrade ${LANG_FRENCH} "Mise à jour"

#LangString Type_Downgrade ${LANG_ENGLISH} "Downgrade"
LangString Type_Downgrade ${LANG_FRENCH} "Retour version précédente"

#LangString Type_Parallel_Installation ${LANG_ENGLISH} "Parallel installation"
LangString Type_Parallel_Installation ${LANG_FRENCH} "Installation parallèle"

#LangString Type_Fresh_Installation ${LANG_ENGLISH} "New installation"
LangString Type_Fresh_Installation ${LANG_FRENCH} "Nouvelle installation"

#LangString Shortcuts ${LANG_ENGLISH} "Shortcuts"
LangString Shortcuts ${LANG_FRENCH} "Raccourcis"

#LangString Create_Shortcut_Start_Menu ${LANG_ENGLISH} "in the Start Menu folder"
LangString Create_Shortcut_Start_Menu ${LANG_FRENCH} "Dans le menu Démarrer"

#LangString Create_Shortcut_Desktop ${LANG_ENGLISH} "on the Desktop"
LangString Create_Shortcut_Desktop ${LANG_FRENCH} "sur le bureau"

#LangString not_selected ${LANG_ENGLISH} "not selected"
LangString not_selected ${LANG_FRENCH} "non sélectionné"

#LangString text_none ${LANG_ENGLISH} "none"
LangString text_none ${LANG_FRENCH} "aucun"

#LangString SecGrpResetConfig_Text ${LANG_ENGLISH} "Reset ALL OpenCPN configuration files"
LangString SecGrpResetConfig_Text ${LANG_FRENCH} "Annuler tous les fichiers de config. OpenCPN"

#LangString SecDeleteConfigFile_Text ${LANG_ENGLISH} "Delete config file (opencpn.ini)"
LangString SecDeleteConfigFile_Text ${LANG_FRENCH} "Supprime le fichier config. (opencpn.ini)"

#LangString SecDeleteChartDB_Text ${LANG_ENGLISH} "Delete Chart Database file (CHRTLIST.DAT)"
LangString SecDeleteChartDB_Text ${LANG_FRENCH} "Supprime le fichier base carte (CHRTLIST.DAT)"

#LangString SecDeleteSENC_Text ${LANG_ENGLISH} "Delete SENC directory"
LangString SecDeleteSENC_Text ${LANG_FRENCH} "Supprime le répertoire SENC"

#LangString SecDeleteCM93_Text ${LANG_ENGLISH} "Delete CM93 Directory"
LangString SecDeleteCM93_Text ${LANG_FRENCH} "Supprime le répertoire CM93"

#LangString SecDeleteLogFile_Text ${LANG_ENGLISH} "Delete Log file (opencpn.log)"
LangString SecDeleteLogFile_Text ${LANG_FRENCH} "Supprime le fichier journal (opencpn.log)"

#LangString SecDeleteNavobjFile_Text ${LANG_ENGLISH} "Delete Navobject file (navobj.xml)"
LangString SecDeleteNavobjFile_Text ${LANG_FRENCH} "Supprime le fichier Routes/Way points/Traces (navobj.xml)"

#LangString SecSetConfig_Text ${LANG_ENGLISH} "OpenCPN Configuration Settings"
LangString SecSetConfig_Text ${LANG_FRENCH} "Configuration de OpenCPN"

#LangString SecSetConfigLang_Text ${LANG_ENGLISH} "Set language to $LANG_NAME"
LangString SecSetConfigLang_Text ${LANG_FRENCH} "Langue sélectionnée :  $LANG_NAME"

#LangString SecSetConfigChartDir_Text ${LANG_ENGLISH} "Select Chart Directories"
LangString SecSetConfigChartDir_Text ${LANG_FRENCH} "Sélectionner un répertoire de cartes"

#LangString SecGrpResetConfig_Desc ${LANG_ENGLISH} "Deletes ALL files and subdirectories in the configuration folder."
LangString SecGrpResetConfig_Desc ${LANG_FRENCH} "Supprime TOUT le contenu du répertoire de configuration."

#LangString SecDeleteConfigFile_Desc ${LANG_ENGLISH} "Deletes the configuration file. This file helds also Routes and Waypoints."
LangString SecDeleteConfigFile_Desc ${LANG_FRENCH} "Supprime le fichier de configuration ."

#LangString SecDeleteChartDB_Desc ${LANG_ENGLISH} "Deletes the Chart DB file."
LangString SecDeleteChartDB_Desc ${LANG_FRENCH} "Supprime le fichier base cartes."

#LangString SecDeleteSENC_Desc ${LANG_ENGLISH} "Deletes the SENC subfolder which helds the Bitmaps and S57 chart cache files."
LangString SecDeleteSENC_Desc ${LANG_FRENCH} "Supprime le sous-répertoire SENC contenant  les caches des Bitmaps et cartes S57."

#LangString SecDeleteCM93_Desc ${LANG_ENGLISH} "Deletes the cm93 subfolder which helds the cached cm93 chart outlines."
LangString SecDeleteCM93_Desc ${LANG_FRENCH} "Supprime le sous-répertoire 'cm93' contenant les  caches des cartes CM93."

#LangString SecDeleteNavobjFile_Desc ${LANG_ENGLISH} "Deletes the file 'navobj.xml' in the config folder. This file contains the Tracks."
LangString SecDeleteNavobjFile_Desc ${LANG_FRENCH} "Supprime  le fichier 'navobj.xml' contenant les Traces."

#LangString SecDeleteLogFile_Desc ${LANG_ENGLISH} "Deletes the OpenCPN log file 'opencpn.log'."
LangString SecDeleteLogFile_Desc ${LANG_FRENCH} "Supprime le fichier journal de OpenCPN 'opencpn.log'."

#LangString SecGrpShortcuts_Desc ${LANG_ENGLISH} "Creates shortcuts in both, the Start Menu and on the Desktop."
LangString SecGrpShortcuts_Desc ${LANG_FRENCH} "Crée un raccourci dans le Menu Démarrage et sur le Bureau."

#LangString SecShortcutStartmenu_Desc ${LANG_ENGLISH} "Creates shortcuts for OpenCPN and the Uninstaller in the Start Menu - configurable on the next page."
LangString SecShortcutStartmenu_Desc ${LANG_FRENCH} "Crée un raccourci pour OpenCPN et pour la désinstallation dans la Menu Démarrage - configurable page suivante."

#LangString SecShortcutDesktop_Desc ${LANG_ENGLISH} "Creates a shortcut for OpenCPN on the Desktop."
LangString SecShortcutDesktop_Desc ${LANG_FRENCH} "Crée un raccourci pour OpenCPN sur le Bureau."

#LangString SecSetConfig_Desc ${LANG_ENGLISH} "Preconfigure the language setting and the chart dir(s) for OpenCPN. When selected a separate page is shown for configuration."
LangString SecSetConfig_Desc ${LANG_FRENCH} "Prédéfinit la langue et le(s) répertoire(s) des cartes pour OpenCPN. Après sélection, une page de configuration sera proposée."

#LangString UnSecDeleteConfigFolder_Desc ${LANG_ENGLISH} "Deletes the config folder and all its files and subdirectories."
LangString UnSecDeleteConfigFolder_Desc ${LANG_FRENCH} "Supprime le répertoire de configuration et tout son contenu."

#LangString Uninstall_Link ${LANG_ENGLISH} "Uninstall OpenCPN 3.3.1117"
LangString Uninstall_Link ${LANG_FRENCH} "Désinstalle  OpenCPN 3.3.1117"

#LangString Page_TypeInst_Title ${LANG_ENGLISH} "Previous Installations"
LangString Page_TypeInst_Title ${LANG_FRENCH} "Installations précédentes"

#LangString Page_TypeInst_Subtitle ${LANG_ENGLISH} "$MULTIPLE_INSTALLS installations of OpenCPN found.$\r$\nChoose how you want to install OpenCPN."
LangString Page_TypeInst_Subtitle ${LANG_FRENCH} "$MULTIPLE_INSTALLS installations d'OpenCPN détectées.$\r$\nChoisir comment vous voulez installer OpenCPN."

#LangString Page_TypeInst_Title_1 ${LANG_ENGLISH} "Already Installed"
LangString Page_TypeInst_Title_1 ${LANG_FRENCH} "Déjà installée"

#LangString Page_TypeInst_Subtitle_1 ${LANG_ENGLISH} "Found installed version of OpenCPN.$\r$\nChoose how you want to install OpenCPN."
LangString Page_TypeInst_Subtitle_1 ${LANG_FRENCH} "OpenCPN déjà installé.$\r$\nChoisir comment vous voulez installer OpenCPN."

#LangString Page_TypeInst_Title_Exists ${LANG_ENGLISH} "Version Already Installed"
LangString Page_TypeInst_Title_Exists ${LANG_FRENCH} "Version déjà installé"

#LangString Page_TypeInst_Subtitle_Exists ${LANG_ENGLISH} "OpenCPN 3.3.1117 is already installed.$\r$\nChoose how you want to install OpenCPN."
LangString Page_TypeInst_Subtitle_Exists ${LANG_FRENCH} "OpenCPN 3.3.1117 est déjà installé.$\r$\nChoisir comment vous voulez installer OpenCPN."

#LangString Label_TypeInst_Reinstall_Text ${LANG_ENGLISH} "OpenCPN 3.3.1117 is already installed on your system. You can reinstall the version now using the previous settings. If you wish to install the program with different settings, cancel installation and un-install the existing version first. Click Next to continue."
LangString Label_TypeInst_Reinstall_Text ${LANG_FRENCH} "OpenCPN 3.3.1117 est déjà installé. Vous pouvez réinstaller cette version en utilisant les paramètres existants. Si vous voulez changer les paramètres , arrêtez l'installation et commencez par désinstaller la version existante . cliquez 'Suivant' pour continuer."

#LangString RB_Reinstall_TypeInst_Text ${LANG_ENGLISH} "Reinstall using previous settings"
LangString RB_Reinstall_TypeInst_Text ${LANG_FRENCH} "Réinstaller avec les paramètres existants"

#LangString Label_TypeInst_Upgrade_Text ${LANG_ENGLISH} "An older version of OpenCPN is installed on your system. Select the installation type you want to perform and click Next to continue."
LangString Label_TypeInst_Upgrade_Text ${LANG_FRENCH} "Une ancienne version de OpenCPN est installée . Choisir l'option d'installation que vous désirez puis cliquer 'Suivant' pour continuer."

#LangString RB_Upgrade_TypeInst_Text ${LANG_ENGLISH} "Upgrade OpenCPN using previous settings (recommended)"
LangString RB_Upgrade_TypeInst_Text ${LANG_FRENCH} "Mise à jour OpenCPN avec les paramètres existants (recommandé)"

#LangString Label_TypeInst_Downgrade_Text ${LANG_ENGLISH} "A newer version of OpenCPN is installed on your system. It is not recommended to downgrade to an older version. Select the installation type you want to perform and click Next to continue."
LangString Label_TypeInst_Downgrade_Text ${LANG_FRENCH} "Une version plus récente d'OpenCPN est installée.Le retour à une  ancienne version n'est pas recommandé . Choisir l'option d'installation que vous désirez puis cliquer 'Suivant' pour continuer."

#LangString RB_Downgrade_TypeInst_Text ${LANG_ENGLISH} "Downgrade OpenCPN using previous settings (recommended)"
LangString RB_Downgrade_TypeInst_Text ${LANG_FRENCH} "Revenir à la version précédente avec les paramètres existants (recommandé)"

#LangString Label_TypeInst_Parallel_Text ${LANG_ENGLISH} "Setup has detected several installed versions of OpenCPN on your system. It is recommended to have only one version of OpenCPN installed at a time. Select the operation you want to perform and click Next to continue."
LangString Label_TypeInst_Parallel_Text ${LANG_FRENCH} "Plusieurs versions ont été détectées. Il est recommandé de n'avoir qu'une seule version installée. Choisir l'opération que vous voulez lancer puis cliquer 'Suivant' pour continuer."

#LangString RB_Parallel_TypeInst_Text ${LANG_ENGLISH} "Parallel Installation (advanced users)"
LangString RB_Parallel_TypeInst_Text ${LANG_FRENCH} "Installation parallèle (utilisateurs avertis!)"

#LangString EmptyString ${LANG_ENGLISH} ""
LangString EmptyString ${LANG_FRENCH} ""

#LangString Show_Readme_Text ${LANG_ENGLISH} "Show Install Log file"
LangString Show_Readme_Text ${LANG_FRENCH} "Montrer le journal d'installation"

#LangString Finish_Link_Text ${LANG_ENGLISH} "OpenCPN website (Internet connection required)"
LangString Finish_Link_Text ${LANG_FRENCH} "Site internet OpenCPN   (Une connexion est requise)"

#LangString Finish_Link_URL ${LANG_ENGLISH} "http://opencpn.info/?ref=installer&user=$USERS_NAME&lang=$REG_LANG&version=3.3.1117"
LangString Finish_Link_URL ${LANG_FRENCH} "http://opencpn.info/?ref=installer&utilisateurr=$USERS_NAME&lang=$REG_LANG&version=3.3.1117"

#LangString Msg_UAC_Admin_Retry ${LANG_ENGLISH} "This $Inst_Or_Uninst requires admin access!$\r$\nPlease log on as an Administrator and try again."
LangString Msg_UAC_Admin_Retry ${LANG_FRENCH} "Cette $Inst_Ou_Desinst nécessite un accès administrateur!$\r$\nConnectez vous en tant qu'administrateur puis reéssayez."

#LangString Msg_UAC_Admin_Abort ${LANG_ENGLISH} "This $Inst_Or_Uninst requires admin privileges, aborting!"
LangString Msg_UAC_Admin_Abort ${LANG_FRENCH} "Cette $Inst_Ou_Desinst demande des privilèges administrateur, abandon!"

#LangString Msg_UAC_Logon_Abort ${LANG_ENGLISH} "Logon service not running, aborting!"
LangString Msg_UAC_Logon_Abort ${LANG_FRENCH} "Connexion impossible, abandon!"

#LangString Msg_UAC_No_Elevation ${LANG_ENGLISH} "Unable to elevate , error $0"
LangString Msg_UAC_No_Elevation ${LANG_FRENCH} "Unable to elevate , error $0"

#LangString Msg_Error_Remove_InstDir ${LANG_ENGLISH} "The installation directory$\r$\n'$INSTDIR'$\r$\ncould not be removed because it is not empty."
LangString Msg_Error_Remove_InstDir ${LANG_FRENCH} "Le répertoire d'installation$\r$\n'$INSTDIR'$\r$\ne peut être supprimé , car il n'est pas vide."

#LangString Msg_Path_Invalid ${LANG_ENGLISH} "The entered path is invalid!$\r$\n$\r$\nPlease check your input."
LangString Msg_Path_Invalid ${LANG_FRENCH} "L'adresse est invalide!$\r$\n$\r$\Veuillez vérifier votre saisie."

#LangString Msg_Path_Installation_Exists ${LANG_ENGLISH} "There already exists an installation of OpenCPN in the given path.$\r$\nIf you wish to use this path, first uninstall the version. Or select another path for this installation."
LangString Msg_Path_Installation_Exists ${LANG_FRENCH} "Une version d'OpenCPN existe à l'adresse indiquée.$\r$\nSi vous voulez utiliser cette adresse, vous devez d'abords désinstaller la version existante. Sinon choisir une autre adresse."

#LangString Msg_Uninstall_Old_Versions ${LANG_ENGLISH} "Setup has detected at least one older version of OpenCPN which is not compatible with the new version control.$\r$\nIt is recommended to first uninstall all of these installations (this does not affect your Config files) and clean-up the Registry.$\r$\nIf you click Yes on the next page a listbox with all installation locations will be shown and you will have the possibility to select the installations you wish to uninstall.$\r$\nIf you are unsure about possible other previous installations than those shown in the listbox, you can also perform a scan of the entire Harddisk (ATTENTION: This may take several minutes) to find all install locations (recommended).$\r$\n$\r$\nDo you want to uninstall the old version(s)?"
LangString Msg_Uninstall_Old_Versions ${LANG_FRENCH}  "Au moins une ancienne version d'OpenCPN incompatible avec la nouvelle version à été détectée.$\r$\nIl est recommandé de commencer par la ou les désinstaller (Vos fichiers de configuration seront conservés)  et de nettoyer le registre.$\r$\nSi vous cliquer 'Oui'  les installations seront affichées sur la page suivante. Vous pourrez choisir celles que vous voulez désinstaller.$\r$\nSi vous pensez qu'il puisse y avoir d'autres versions que celles affichées Vous pouvez exécuter un balayage complet du disque dur (ATTENTION: Cela peut prendre plusieurs minutes) pour trouver toutes les versions (recommandé)?"

#LangString Msg_Not_All_Old_Uninstalled ${LANG_ENGLISH} "You have not uninstalled all previous installations.$\r$\n$\r$\nDo you want to continue anyway?"
LangString Msg_Not_All_Old_Uninstalled ${LANG_FRENCH} "Les anciennes versions ne sont pas toutes désinstallées.$\r$\n$\r$\nVoulez vous continuer?"

#LangString Msg_Uninstaller_Param_Help ${LANG_ENGLISH} "Uninstaller Help$\r$\n$\r$\nUse the command line or a shortcut to run the uninstaller:$\r$\n$INSTDIR\Uninstall 3.3.1117.exe [/?] | [/S], [/type=X]$\r$\n$\r$\nParameters:$\r$\n/?$\tshows this help message box$\r$\n/S$\truns the uninstaller in silent mode$\r$\n$\twill do a full uninstall if called without any type parameter$\r$\n/type=X$\tX must be a digit between 1 and 5$\r$\n$\t1 = Removes only the program files$\r$\n$\t2 = Removes only the program files + preconfig RegKey$\r$\n$\t3 = Removes the program files + config folder$\r$\n$\tpreserves the preconfig RegKey$\r$\n$\t4 = Deletes only the config folder$\r$\n$\t5 = Deletes only the preconfig RegKey$\r$\n$\r$\nEntering any other number or value will quit the uninstaller with a message box without removing or deleting anything."
LangString Msg_Uninstaller_Param_Help ${LANG_FRENCH} "Aide désinstalleur$\r$\n$\r$\nUtiliser une ligne de commande ou un raccourci pour lancer la désinstallation:$\r$\n$INSTDIR\Uninstall3.3.1117.exe [/?] | [/S], [/type=X]$\r$\n$\r$\nType:$\r$\n/?$\tAffiche l'aide$\r$\n/S$\tLance la désinstallation en arrière plan$\r$\n$\tUne désinstallation complète sera exécutée si lancée sans aucune option$\r$\n/Option=X$\tX doit être choisie entre 1 et 5$\r$\n$\t1 = Supprime uniquement le programme$\r$\n$\t2 = Supprime le programme + les clefs du registre$\r$\n$\t3 = Supprime le programme + le répertoire de configuration$\r$\n$\tConcerve les clefs du registre$\r$\n$\t4 = Supprime uniquement le répertoire de configuration$\r$\n$\t5 = Supprime uniquement les clefs du registre$\r$\n$\r$\nUn autre choix  stoppera la désinstallation et affichera un message sans aucune action."

#LangString Msg_Unknown_Type_Param ${LANG_ENGLISH} "The entered value '$9' for the type parameter is not valid!$\r$\nUse $INSTDIR\Uninstall 3.3.1117.exe /?$\r$\nto see a list of valid values."
LangString Msg_Unknown_Type_Param ${LANG_FRENCH} "la valeur '$9' est une option invalide!$\r$\nUtilisez $INSTDIR\Uninstall 3.3.1117.exe /?$\r$\npour voir la liste des options valides."

;added on 2010-10-16
#LangString SecGrpShortcuts_Text ${LANG_ENGLISH} "Create Shortcuts"
LangString SecGrpShortcuts_Text ${LANG_FRENCH} "Créer les raccourcis"

#LangString SecShortcutStartmenu_Text ${LANG_ENGLISH} "Add Shortcuts to the Start Menu"
LangString SecShortcutStartmenu_Text ${LANG_FRENCH} "Ajouter les raccourcis au menu Démarrage"

#LangString SecShortcutDesktop_Text ${LANG_ENGLISH} "Create Shortcut on the Desktop"
LangString SecShortcutDesktop_Text ${LANG_FRENCH} "Créer un raccourci sur le Bureau"

#LangString Installation_Delete_Config ${LANG_ENGLISH} "Delete (existing) Config Subdirectories/ Files"
LangString Installation_Delete_Config ${LANG_FRENCH} "Supprimer le contenu du répertoire de configuration"

;added on 2010-10-17
#LangString Banner_Scan_Drive ${LANG_ENGLISH} "Scanning Drive $9"
LangString Banner_Scan_Drive ${LANG_FRENCH} "Balayage du disque $9"

#LangString Banner_Installation_Found_Singular ${LANG_ENGLISH} "Found $counter_tmp installation"
LangString Banner_Installation_Found_Singular ${LANG_FRENCH} "$counter_tmp installation trouvée"

#LangString Banner_Installation_Found_Plural ${LANG_ENGLISH} "Found $counter_tmp installations"
LangString Banner_Installation_Found_Plural ${LANG_FRENCH} "$counter_tmp installations trouvées"

;added on 2010-10-18
#LangString UnSecDeleteConfigFolder_Text ${LANG_ENGLISH} "Delete Config folder"
LangString UnSecDeleteConfigFolder_Text ${LANG_FRENCH} "Supprimer le répertoire de configuration"

;added on 2010-10-21
#LangString Uninstall_Folder_Name ${LANG_ENGLISH} "Uninstall"
LangString Uninstall_Folder_Name ${LANG_FRENCH} "Désinstallation"

;added on 2011-06-13
#LangString ^NameDA ${LANG_ENGLISH} "OpenCPN Version 3.3.1117"
LangString ^NameDA ${LANG_FRENCH} "OpenCPN Version 3.3.1117"

;added on 2011-07-24
#LangString License_Message ${LANG_ENGLISH} "OpenCPN is software libre. You are encouraged and legally entitled to copy, reinstall, modify, and redistribute this program for yourself and your friends under the terms of the GPL. $\r$\nSmooth sailing!"
LangString License_Message ${LANG_FRENCH} "OpenCpn est un logiciel libre. Vous êtes encouragés et légalement autorisés à copier, réinstaller, modifier et redistribuer ce programme, pour vous-même et vos amis, selon les termes de la licence GPL. Bonnes navigations!"

;Needs to be translated:
#LangString Installer_Unsupported_XP_SP_Version ${LANG_ENGLISH} "Your Windows XP Service Pack version is not supported (it's too old)!$\r$\nYou need at least Windows XP SP3."
LangString Installer_Unsupported_XP_SP_Version ${LANG_FRENCH} "Votre version de Windows XP Service Pack version n'est pas supportée (car trop ancienne)!$\r$\nVous avez besoin d'au moins Windows XP SP3."
